...
const todosService = {
  async getAll() {
    ...
    console.log("Ответ сервера на получение всех задач:", data);

    if (!response.ok) {
      console.error("Ошибка при получении списка задач:", data);
  ...
  async update(todoId, completed) {
    console.log(
      `Запрос на обновление задачи ${todoId} на ${
    ...
    const data = await response.json();
    console.log(`Ответ сервера на обновление задачи ${todoId}:`, data);
    return data;
  },
